...
function UserInfo(){
    ...
    //어떤 클릭이 없오도 UserInfo 페이지 들어오면 자동으로 실행되는 효과
    useEffect(() =>{
        ...
        // URLSearchParams 가져온 수많은 값 중 에서 키 이름이 access_token인 값만 가져오겠다
    
        // get 을 이용해서 userinfo 정보 가져오기
        // String redirectUrl = "http://localhost:3000/userinfo?access_token=" + accessToken;
        // 자바에서는 userinfo?access-tiken= 뒤에 + 를 붙여 변수를 사용했지만
        // 자바스크립트에서는 ``를 사용해서 const accessToken =a.get('access_token);
        // 가져옴



        //만약에 accessTioken 값이 존재하면 axios 발동
        if(accessToken){

            axios.get(`/userinfo?access_token=${accessToken}`)
            //.then((res) => { .then((res) => { (res)} 막아버리면 =>이후로는 res선언되지 않은
            //지역변수명이 되기 떄문에 res를 찾을 수 없는 것
            .then(response => {
    ...
    //access_token 값을 가져오면 useEffct를 사용하겠다.
    ...
    return (
        ...
        {userInfo ? (
            ...
            <div>
                ...
                <input type="nickname" value={userInfo.response.nickname} disabled/>
                <input type="name" value={userInfo.response.name} disabled/>
                <input type="gender" value={userInfo.response.gender} disabled/>
